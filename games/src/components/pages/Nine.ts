...
class Nine implements Page {
  ...
  public async render(): Promise<string> {
    ...
    for (let i=0; i<this.playerCards.length; i+=1) {
      myCards += `<div class="nine-card-my photo${this.playerCards[i]}" style="width: ${this.mainWidth*0.8/18}px" id="myCard${this.playerCards[i]}"></div>`
    }
  ...
  private startNine():void {
    ...
    (document.getElementById('card33') as HTMLElement).setAttribute('class', 'photo33');
  ...
  private moveNine(currentPlayer): void {
    ...
    if (currentPlayer === 0) {
      ...
      if(duplicatedValues.length > 0) {
        ...
        (document.querySelectorAll('.bot-cards')[0] as HTMLElement).innerText = `${this.bot1Cards.length}`;
        (document.getElementById(`card${duplicatedValues[0]}`) as HTMLElement).setAttribute('class', `photo${duplicatedValues[0]}`);
  ...
  public playerMove(event): void {
    ...
    this.currentPlayer =  this.currentPlayer == this.players - 1? 0 : 1;
    (document.getElementById(`card${currentNumber}`) as HTMLElement).setAttribute('class', `photo${currentNumber}`);
